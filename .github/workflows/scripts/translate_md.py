...
This script follows the standard Sphinx gettext i18n workflow:
  ...
  4. Save .po files to locale/en/LC_MESSAGES/ with same subdirectory structure as docs/zh/
...
Directory layout:
    ...
    locale/en/LC_MESSAGES/ mirrors the same subdirectory structure as docs/zh/
...
# Sphinx generates .pot files under locale/ with the same subdirectory
# structure as the source, so a file like docs/zh/triton_api/index.md
# produces locale/triton_api/index.pot.
...
def _source_path_for_pot(pot_path: Path) -> Optional[Path]:
    """Given a .pot file path, determine the original source file path.

    e.g. locale/triton_api/foo.pot -> docs/zh/triton_api/foo.md
         locale/zh/LC_MESSAGES/quick_start.pot -> docs/zh/quick_start.md
    """
    try:
        rel = pot_path.relative_to(LOCALE_DIR)
    except ValueError:
        return None
    # If already under LC_MESSAGES, the first two parts are language/LC_MESSAGES/
    if "LC_MESSAGES" in rel.parts:
        # locale/zh/LC_MESSAGES/quick_start.pot -> zh/LC_MESSAGES/quick_start.pot
        # We want the source relative path, which is everything after LC_MESSAGES/
        lc_index = rel.parts.index("LC_MESSAGES")
        source_rel = Path(*rel.parts[lc_index + 1:])
    else:
        # locale/triton_api/foo.pot -> triton_api/foo.pot
        source_rel = rel
    return ZH_DIR / source_rel.with_suffix('.md')
...
def _is_excluded(pot_path: Path) -> bool:
    ...
    try:
        ...
        return False
    # Check first directory component (source tree structure is preserved)
    first_part = rel.parts[0] if rel.parts else ""
    # Skip the language/LC_MESSAGES/ prefix if already organized
    if first_part in ("zh", "en") and len(rel.parts) >= 3 and rel.parts[1] == "LC_MESSAGES":
        # locale/zh/LC_MESSAGES/python-api/foo.pot -> check python-api
        if len(rel.parts) >= 3:
            first_part = rel.parts[2]
    # Also check the source file name stem
    return first_part in EXCLUDED_DIRS


def _is_excluded_before_organize(pot_path: Path) -> bool:
    """Check if a .pot file (directly under locale/) comes from excluded content.

    Before reorganization, .pot files are directly under locale/ with the
    source subdirectory structure preserved. So docs/zh/triton_api/index.md
    produces locale/triton_api/index.pot.
    """
    try:
        rel = pot_path.relative_to(LOCALE_DIR)
        ...
        return False
    # First component is the source subdirectory (e.g. triton_api, python-api)
    first_part = rel.parts[0] if rel.parts else ""
...
class PoTranslator:
    ...
    async def translate_file(self, pot_path: Path) -> bool:
        ...
        The .po file is placed in locale/en/LC_MESSAGES/ maintaining the same
        subdirectory structure as locale/zh/LC_MESSAGES/.
        """
...
# ---------------------------------------------------------------------------
# POT discovery and organization helpers
# ---------------------------------------------------------------------------
...
def _organize_pot_files():
    ...
    source directory structure. Since locale_dirs = ['../locale/'] in
    the Sphinx config, Sphinx places .pot files at locale/ directly.

    This function moves them to locale/zh/LC_MESSAGES/ (the standard
    gettext layout expected by Sphinx when reading .po files).
    ...
    for pot_file in sorted(LOCALE_DIR.rglob("*.pot")):
        ...
        if _is_excluded_before_organize(pot_file):
            pot_file.unlink()
            discarded += 1
            continue
        target = POT_DIR / rel
