...
`max_autotune` is an enhanced auto-tuning decorator provided by Triton-Ascend, designed to simplify the code writing for multi-parameter tuning. Unlike the community version `autotune`, which requires users to manually enumerate all `triton.Config` entries, `max_autotune` allows users to provide only a few basic configurations (such as block sizes) and automatically incorporates related compiler options (such as `num_stages`, `enable_hivm_auto_cv_balance`, etc.) into the search space for optimal combinations. Users can also explicitly control the search scope through parameter lists.
...
def test_max_autotune():
    ...
    # Basic configuration: only provide block sizes, other tuning parameters are auto-generated by the decorator
    base_configs = [
...
Users can explicitly specify the compiler options to tune and their value lists via **tuning_params**; parameters not specified will use built-in default values. The following example demonstrates how to perform a combined search over multiple parameters.
...
def test_max_autotune():
    ...
    @max_autotune(
        ...
        # The following parameters are optional tuning lists; built-in defaults are used if not provided
        num_stages=[1, 2],
