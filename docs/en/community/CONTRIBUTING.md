...
- Read [README.md](https://github.com/triton-lang/triton-ascend/blob/main/README.md) for project information and building the development environment.
...
- Static Check Failure
   ...
   Based on the prompt information, find the abnormal information in the code and resolve it.

- CI Pipeline Failure

   Based on the prompt information, find the test cases that caused the CI pipeline failure, check the reason, resolve it, and re-run the CI pipeline.

<h2 id="issue-guidelines">ISSUE Guidelines</h2>

A good way to contribute to the project is to send a detailed report when you encounter a problem. We always appreciate well-written, thorough bug reports and will be very grateful for them!
...
- What kind of issue are you reporting? Add appropriate labels to highlight it on the issue dashboard.
- What happened?
...
- Only proceed with forking, development, and submitting a PR after consensus is reached on the issue discussion and design proposal review.
- No PR is allowed until 2+ LGTM (Looks Good To Me) are received from Approvers. Note that reviewers are not allowed to add LGTM to their own PRs.
