...
All submissions must include a `Signed-off-by:` line, which can be added automatically using `git commit -s`:

```bash
git commit -s -m "your commit message"
```

This will automatically append a line `Signed-off-by: Your Name <your.email@example.com>` at the end of the commit message, confirming that you acknowledge the origin and authorization of the contribution.
...
1. Fork the Triton Ascend Project
   ...
   Before submitting your own code to the Triton Ascend project, ensure you have forked the Triton Ascend project to your own repository. You will develop on your forked project and merge it into the Triton Ascend project via Pull Request. This means there is parallel development between the Triton Ascend repository and your own repository, so please ensure consistency between the repositories.
...
4. Self-Test Code Changes
   ...
   Write test case code for your developed code under the `ascend/examples/pytest_ut` path in your local code branch, and verify your test scripts in the local environment to ensure your changes pass the tests.
...
6. Create a Pull Request to the Triton Ascend Main Repository
   ...
   After pushing the code to your remote repository, you need to create a new Pull Request between your new branch and the Triton Ascend main branch. Once the merge request is created, "Jenkins CI" will automatically be set up to build pipeline tests for you. Please merge your Pull Request into the upstream main branch as soon as possible to reduce merge risks.
...
- CI Pipeline Failure
   ...
   Please find the test cases that caused the CI pipeline failure based on the prompt information, check the reason, resolve it, and rerun the CI pipeline.

<h2 id="issue-specification">ISSUE Specification</h2>

A good way to contribute to the project is to send detailed reports when encountering problems. We always greatly appreciate detailed and thorough bug reports and will be very grateful for them!
...
- If you find an unresolved issue that you intend to solve, please comment on the issue to let others know you will be responsible for it.
- If an issue has been open for a while, please perform a pre-check before resolving it.
- If you solve an issue you reported yourself, let others know before closing the issue.
...
- Only after reaching consensus on the issue discussion and design proposal review should you proceed with fork development and submit a PR.
- No PR is allowed until 2+ LGTM (Looks Good To Me) are received from Approvers. Please note that reviewers are not allowed to add LGTM on their own PRs.
