...
- Static Check Failure
   ...
   Based on the prompt information, find and resolve the anomalies in the code.

- CI Pipeline Failure

   Based on the prompt information, find the test cases that caused the CI pipeline failure, check the reasons, resolve them, and re-run the CI pipeline.
...
- Only proceed with fork development and submit a PR after reaching consensus through issue discussion and design proposal review.
- No PR is allowed until 2+ LGTM (Looks Good To Me) are received from Approvers. Note that approvers are not allowed to add LGTM on their own PRs.
- After sufficient discussion on the PR, the PR will be merged, rejected, or abandoned based on the discussion results.
