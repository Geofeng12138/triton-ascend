...
- [Developer Certificate of Origin (DCO)](#developer-certificate-of-origin-dco.md)
- [Development Guide](#development-guide.md)
  ...
  - [ISSUE Specification](#issue-specification.md)
...
<h2 id="developer-certificate-of-origin-dco.md">Developer Certificate of Origin (DCO)</h2>

All commits must include a `Signed-off-by:` line, which can be added automatically using `git commit -s`:

```bash
git commit -s -m "your commit message"
```

This will automatically append a line `Signed-off-by: Your Name <your.email@example.com>` at the end of the commit message, confirming your acknowledgment of the contribution's origin and authorization.
...
3. Develop Code in the Local Environment
   ...
   To avoid inconsistencies between multiple branches, create a new local development branch for developing new features:
...
4. Self-Test Code Changes
   ...
   After completing code changes, please check if your changes pass the tests:

   Write test case code for your developed code under the `ascend/examples/pytest_ut` path of your local code branch, and verify your test scripts in the local environment to ensure your changes pass the tests.
...
- Compilation Failure
   ...
   Please check the reason for the compilation failure based on the prompt information, resolve it, and recompile.

- Static Check Failure

   Please find and resolve the exception information in the code based on the prompt information.

- CI Pipeline Failure

   Please find the test cases that caused the CI pipeline failure based on the prompt information, check the reason, resolve it, and re-run the CI pipeline.

<h2 id="issue-specification.md">ISSUE Specification</h2>

A good way to contribute to the project is to send a detailed report when you encounter a problem. We always greatly appreciate detailed and thorough bug reports and will be very grateful for them!
...
<h2 id="submitting-a-pr.md">Submitting a PR</h2>
...
- Only proceed with forking, development, and submitting a PR after consensus is reached in the issue discussion and design proposal review.
...
- After sufficient discussion of the PR, it will be merged, rejected, or abandoned based on the discussion results.

## Notes

- Avoid any irrelevant changes.
- Ensure your commit history is concise and orderly.
- Please rebase with the latest upstream repository code before creating a PR.
