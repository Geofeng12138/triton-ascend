...
It is recommended that users enable ASLR (**Address Space Layout Randomization**) on the system, with level 2 (full randomization mode) preferred. Configuration can be performed as follows:
...
2. Permission control should be implemented during the user installation and usage process. Refer to the [File Permission Reference](#file-permission-reference) for suggested settings.
...
Triton-Ascend supports source code compilation and installation. During compilation, dependent third-party libraries will be downloaded and build shell scripts will be executed. Temporary program files and build directories will be generated during the compilation process. Users can manage permissions on files within the source code directory as needed to reduce security risks.
