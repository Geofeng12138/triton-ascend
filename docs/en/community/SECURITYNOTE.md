...
It is recommended that users enable ASLR (**Address Space Layout Randomization**) in the system, with level 2 (full randomization mode) preferred. The configuration can be performed as follows:
...
2. Permission control should be exercised during the installation and usage process. It is recommended to configure settings according to the [File Permission Reference](#file-permission-reference).
...
Triton-Ascend supports source code compilation and installation. During compilation, it will download dependent third-party libraries and execute build shell scripts. Temporary program files and build directories will be generated during the compilation process. Users can manage permissions on files within the source code directory as needed to reduce security risks.
