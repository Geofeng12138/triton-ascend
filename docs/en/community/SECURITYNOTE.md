...
It is recommended that users enable ASLR (**Address Space Layout Randomization**) on their systems, with level 2 (full randomization mode) being the preferred setting. Configuration can be performed as follows:
...
2. Permission control should be exercised during the installation and usage process. It is recommended to configure permissions according to the [File Permission Reference](#file-permission-reference).
...
Triton-Ascend supports installation via source code compilation. During compilation, dependent third-party libraries will be downloaded and build shell scripts executed. Temporary program files and build directories will be generated during this process. Users can manage permissions on files within the source code directory as needed to mitigate security risks.
...
| Open Source Import | <https://repo.huaweicloud.com>                                            | scripts/prepare_build.sh                           | <https://repo.huaweicloud.com/repository/pypi/simple>                                                | Used for configuring pybind11 download link |
...
| Open Source Import | <https://triton-ascend-artifacts.obs.myhuaweicloud.com> | setup.py |`https://triton-ascend-artifacts.obs.myhuaweicloud.com/llvm-builds/{name}.tar.gz` | Used for downloading pre-compiled LLVM tools |
