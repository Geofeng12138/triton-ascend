...
It is recommended to enable ASLR (**Address Space Layout Randomization**) on the system, with level 2 (full randomization mode) preferred. Configuration can be done as follows:
...
2. Permission control should be applied during the installation and usage process. It is recommended to configure permissions based on the [File Permission Reference](#file-permission-reference).
...
Triton-Ascend supports source code compilation and installation. During compilation, dependent third-party libraries will be downloaded and build shell scripts will be executed. Temporary program files and build directories will be generated during the process. Users can manage permissions on files within the source code directory as needed to reduce security risks.
