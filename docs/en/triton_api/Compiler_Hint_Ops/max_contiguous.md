...
**`values` describes the contiguity characteristics of each dimension, so the dimension of `values` must be the same as the dimension of `input`.
Note the dimension reduction case when the last dimension of `shape` is `1`.**

For example: a two-dimensional `input` corresponds to the general `values` input parameter `[1,1]`.
