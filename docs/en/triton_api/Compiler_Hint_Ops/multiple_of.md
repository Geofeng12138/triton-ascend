...
**`values` describes the divisibility characteristics of the first value in each dimension, so the dimensionality of `values` must be the same as that of `input`.
