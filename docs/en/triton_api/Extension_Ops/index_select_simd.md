...
1. **dim restriction:** The index_select operation is not supported on the tail axis (last dimension). `dim` must satisfy `dim < len(src_shape) - 1`
2. **Data type restrictions:** uint16/uint32/uint64/float8/float64 data types are currently not supported
3. **Index out of bounds:** Index out-of-bounds in `index` is not checked. The user must ensure the validity of indices.
