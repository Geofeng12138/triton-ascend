...
| dim | int | Yes | The dimension along which to perform the index_select operation. Value range [0, len(src_shape)-2], **does not support the tail axis** (last dimension) |
...
1. **dim Constraint:** The index_select operation is not supported on the tail axis (last dimension). `dim` must satisfy `dim < len(src_shape) - 1`
2. **Data Type Constraint:** uint16/uint32/uint64/float8/float64 data types are currently not supported
3. **Index Out of Bounds:** Out-of-bounds indices in `index` are not checked. The user must ensure the validity of the indices.
