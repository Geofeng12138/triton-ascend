...
`parallel` is an iterator specifically designed for multi-core parallel execution. It inherits from the `range` class and provides explicit multi-core parallel semantics.
...
> **Note**: `parallel` has removed the following parameters compared to `range`:
>
...
When `bind_sub_block` is true, the IR reflects a distinction from `range`. Whether the functionality is actually implemented requires verification.
