...
| `scope`        | `str`, optional     | The thread scope observing the synchronization effects of the atomic operation.<br>Accepted values are "gpu" (default), "cta" (cooperative thread array, thread block), or "sys" (representing "SYSTEM").<br>We only support "gpu". |
...
| ---------------- | ------------------------------------------------------------------------------------- | ------------------------------ |
| `sem`            | Accepted values in the official community configuration are "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE"), and "relaxed".<br>We only support "acq_rel". | Under development |
| `scope`          | Accepted values are "gpu", "cta", or "sys".<br>We only support "gpu".                 | Under development |
