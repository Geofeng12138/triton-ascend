...
| `scope` | `str`, optional | The thread scope for observing the synchronization effects of the atomic operation.<br>Accepted values are "gpu" (default), "cta" (cooperative thread array, thread block), or "sys" (representing "SYSTEM").<br>We only support "gpu".                                            |
...
> Relative capability gaps compared to the community that cannot be implemented.
