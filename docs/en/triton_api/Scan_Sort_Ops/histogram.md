...
Note: The current triton 3.2 version does not support `mask`; support will be added in a future version. The `input` range is restricted to [0, num_bins-1]; full range support will be added in a future version.
...
> Relative to community capabilities, missing but can be developed to support.