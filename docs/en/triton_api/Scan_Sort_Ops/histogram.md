...
Description: Computes a histogram with `num_bins` bins based on `input`, each bin has a width of 1 and starts at 0.
Prototype:
...
Note: The current triton 3.2 version does not support `mask` yet; support will be added in a future version. The `input` range is limited to [0, num_bins-1]; full range support will be added in a future version.
