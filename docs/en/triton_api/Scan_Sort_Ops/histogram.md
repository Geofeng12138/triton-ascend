...
Note: The current triton 3.2 version does not support `mask`; support will be added in future versions. The `input` range is restricted to [0, num_bins-1]; full range support will be added in future versions.
...
> Relative community capability missing but can be developed to support