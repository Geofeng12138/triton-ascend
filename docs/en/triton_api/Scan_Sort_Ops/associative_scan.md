...
> `reverse=True` indicates whether to apply the associative scan along the reverse direction of the axis. This feature requires data alignment when loading with `tl.load`, meaning no mask is used to filter out excess data indices, as shown in the following example code:
