...
| `combine_fn` | `Callable` | Function used to combine two scalar tensor groups (must be decorated with `@triton.jit`) |
...
| `_generator` | `Optional[Generator]` | Reserved parameter, external calls not supported yet |
...
> `reverse=True` indicates whether to apply the associative scan in the reverse direction along the axis. This feature requires data alignment when loading with `tl.load`, meaning no mask is used to filter out excess data indices, as shown in the following example code:
