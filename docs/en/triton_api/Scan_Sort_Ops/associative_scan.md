...
| `_semantic` | `Optional[str]` | Reserved parameter, external calls not currently supported |
| `_generator` | `Optional[Generator]` | Reserved parameter, external calls not currently supported |
...
> `reverse=True` determines whether to apply the associative scan in the reverse direction along the axis. This functionality requires data alignment when loading with `tl.load`, meaning no mask is used to filter out excess data indices, as shown in the following example code:
