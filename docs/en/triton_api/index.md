...
|[index_select High-Performance Interface](./Extension_Ops/index_select_simd.md) | Gathers multiple indices in parallel on non-trailing axis dimensions and moves data zero-copy from global memory (GM) directly to the correct position in the unified buffer (UB) in units of tiles. This operation is equivalent to a high-performance implementation of `torch.index_select`, suitable for scenarios such as embedding layer lookups and sparse index access. |
...
|[not](./Logical_Ops/not.md) | Performs bitwise NOT on the values of a tensor. |
|[invert](./Logical_Ops/invert.md) | Performs bitwise inversion on each value of a tensor. |
|[lshift](./Logical_Ops/lshift.md) | Performs a left shift on a tensor by a given number of shifts. |
|[rshift](./Logical_Ops/rshift.md) | Performs a right shift on a tensor by a given number of shifts. |
...
|[histogram](./Scan_Sort_Ops/histogram.md) |Computes a histogram with num_bins bins based on the input tensor, each bin has a width of 1 and starts at 0 |
|[sort](./Scan_Sort_Ops/sort.md) |Sorts a tensor along the specified dimension |
