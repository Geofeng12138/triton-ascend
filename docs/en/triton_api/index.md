...
|[index_select High-Performance Interface](./Extension_Ops/index_select_simd.md) | Gathers multiple indices in parallel on non-trailing axes and copies data zero-copy from global memory (GM) directly to the correct position in the unified buffer (UB) in units of tiles. This operation is equivalent to a high-performance implementation of `torch.index_select`, suitable for scenarios like embedding layer lookups and sparse index access. |
...
|[div_rn](./Math_Ops/div_rn.md) | Computes the element-wise exact division of x and y (rounded to the nearest value per IEEE standard) |
...
|[sqrt_rn](./Math_Ops/sqrt_rn.md) | Computes the element-wise exact square root of x (rounded to the nearest value per IEEE standard) |
