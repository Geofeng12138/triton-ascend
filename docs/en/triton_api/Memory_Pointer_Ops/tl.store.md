...
| `mask` | `int1` or `tensor<int1>` | Optional parameter, can only be passed when `pointer` does not originate from `tl.make_block_ptr`<br>If `mask[i]==False`, `value[i]` will not be stored at the address pointed to by `pointer[i]`; if `True`, it is stored normally.<br>If `pointer` originates from `tl.make_block_ptr`, `mask` must be `None`. |
...
| Generalization issue when used with branch and loop statements | Currently, the computation of `pointer` and `mask` in `tl.load` may cause compilation issues if it involves complex loops and branch statements. | Expose issues through extensive generalization testing, resolve iteratively. |
