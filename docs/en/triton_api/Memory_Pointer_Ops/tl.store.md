...
Description: Stores a Tensor/Scalar from the UnifiedBuffer back to GlobalMemory at the address pointed to by `pointer`.
...
| --------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `pointer`       | `triton.PointerType` <br> or `tensor<triton.PointerType>` <br> or `triton.PointerType<tensor>` (from `tl.make_block_ptr`) | Pointer to the address in GM where the data should be stored.                                                                    |
...
| `boundary_check`| `tuple(int)`       | Optional parameter. Can only be passed when `pointer` is from `tl.make_block_ptr`.<br>An integer tuple indicating the dimensions for which boundary checking is required. |
...
| `_semantic`     | -                  | Reserved parameter. External calls are not supported at this time.                                                               |
...
Compared to GPU, Ascend lacks support for uint8, uint16, uint32, uint64, and fp64 (hardware limitation). The `eviction_policy` and `cache_modifier` functionalities are not yet complete on the NPU.

| Difference Point                          | Description                                                                                                                    | Resolution Path                              |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------- |
| Generalization issue with discrete masks  | The current handling of discrete masks in `store` involves decomposing the store into atomic {load, select, store} operations, which has generalization issues in corner cases. | Expose issues through extensive generalization testing, resolve iteratively. |
| Generalization issue with branches/loops  | The computation of `pointer` and `mask` in `tl.load` may encounter compilation issues if it involves complex loops and branch statements. | Expose issues through extensive generalization testing, resolve iteratively. |
