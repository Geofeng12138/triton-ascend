...
| ---------- | ----------- | ---------- |
| Generalization issue with discrete masks | Currently, the handling of discrete masks in `store` involves decomposing the store into atomic {load, select, store} operations, which has certain generalization issues in corner cases. | Expose issues through extensive generalization testing, resolve iteratively. |
| Generalization issue when used with loops and conditional statements | Currently, the computation of `pointer` and `mask` in `tl.load` may encounter compilation issues if it involves complex loops and conditional statements. | Expose issues through extensive generalization testing, resolve iteratively. |
