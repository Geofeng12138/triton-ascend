...
| ---------------- | ------------------- | ----------------------------------------------------------------------- |
| `pointer`        | `triton.PointerType` <br> or `tensor<triton.PointerType>` <br> or `triton.PointerType<tensor>` (from `tl.make_block_ptr`) | Pointer to the target address in GM for storage                         |
| `value`          | `tensor` or `scalar` | The value to be stored, supports implicit broadcasting and implicit type conversion |
| `mask`           | `int1` or `tensor<int1>` | Optional parameter, can only be passed when `pointer` is not from `tl.make_block_ptr`.<br>If `mask[i]==False`, `value[i]` will not be stored at the address pointed to by `pointer[i]`; if `True`, storage proceeds normally.<br>If `pointer` is from `tl.make_block_ptr`, `mask` must be `None`. |
...
> Capabilities missing relative to the community and unimplementable.
...
| Generalization issue with loops and branches | Currently, the computation of `pointer` and `mask` in `tl.load`, if involving complex loops and branch statements, may cause compilation issues. | Expose issues through extensive generalization testing, resolve iteratively. |
