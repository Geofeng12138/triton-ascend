...
1. If `pointer` is a single pointer:
   - `value` and `mask` must be a scalar.
...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| Generalization issue of discrete mask | Currently, the handling of discrete masks in `store` involves decomposing the store into atomic {load, select, store} operations, which has certain generalization issues in corner cases. | Expose issues through extensive generalization tests, resolve iteratively. |
| Generalization issue when used with branch/loop statements | Currently, the computation process of `pointer` and `mask` for `tl.load` may encounter compilation issues if it involves complex loops and branch statements. | Expose issues through extensive generalization tests, resolve iteratively. |
