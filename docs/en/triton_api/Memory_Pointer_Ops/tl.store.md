...
| --------------- | ------------------- | ----------------------------------------------------------------------- |
| `pointer`       | `triton.PointerType` <br> or `tensor<triton.PointerType>` <br> or `triton.PointerType<tensor>` (from `tl.make_block_ptr`) | Pointer to the address in GM where data is to be stored |
...
| `boundary_check`| `tuple(int)`         | Optional parameter, can only be passed when `pointer` originates from `tl.make_block_ptr`<br>An integer tuple indicating the dimensions for which boundary checking is required |
...
| ---------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------- |
| Generalization issue with discrete masks | Currently, the handling of discrete masks in store involves decomposing store into atomic {load, select, store}, which has certain generalization issues in corner cases | Expose issues through extensive generalization testing, resolve iteratively |
| Generalization issue with branches and loops | Currently, the computation of `pointer` and `mask` for `tl.load` may encounter compilation issues if it involves complex loops and branch statements | Expose issues through extensive generalization testing, resolve iteratively |
