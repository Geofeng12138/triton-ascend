...
| `strides`      | `List[tensor]`      | List of strides for each dimension of the tensor. Constraints: - The leading dimensions must be multiples of 16 bytes - The last dimension must be contiguous |
| `block_shape`  | `List[constexpr]`   | Shape of the block to be loaded/stored from global memory                                                                        |
| `_semantic`    | -                   | Reserved parameter, external calls are not supported currently                                                                   |

Return Value:
`tensor_descriptor`: A tensor descriptor object (cannot be used directly for arithmetic operations; must be used with `load` / `store`)
