...
Description: Returns a Tensor/Scalar whose values are loaded from the GlobalMemory location pointed to by the `pointer` argument.
...
| `eviction_policy` | `str` | Controls the eviction policy of NVIDIA PTX, has no effect on Ascend hardware. |
| `volatile` | `str` | Controls the volatile option of NVIDIA PTX, has no effect on Ascend hardware. |
...
1. If `pointer` is a single pointer:
   ...
   - `boundary_check` and `padding_option` are not allowed to be passed in this case
...
| Generalization issues when used with branch and loop statements | The computation process of `pointer` and `mask` in `tl.load` may cause compilation issues if it involves complex loops and branch statements. | Expose issues through extensive generalization testing, solve iteratively |
