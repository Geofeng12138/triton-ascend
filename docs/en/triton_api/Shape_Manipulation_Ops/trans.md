...
Transposes the dimensions of a tensor according to the `dims` parameter, without modifying the tensor's data, only changing the order of dimensions. A specially optimized transpose operation.
...
- Does not modify the tensor's data, only changes the order of dimensions
