...
Transposes the dimensions of a tensor according to the `dims` parameter, without changing the tensor's data, only reordering the dimensions. A specially optimized transpose operation.
...
- Does not change the tensor's data, only reorders the dimensions
