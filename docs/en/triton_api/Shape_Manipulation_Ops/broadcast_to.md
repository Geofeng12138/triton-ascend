...
Broadcasts a tensor to a target shape, automatically handling dimension alignment. The broadcast operation does not copy data; instead, it achieves this by modifying the tensor's shape and strides.
...
Unlike `broadcast`, the `broadcast_to` implementation in the Triton community requires that the rank of the tensor's shape must be equal to the rank of the target shape.
