...
- Preserves data unchanged, only modifying the tensor's shape information.
...
- **Memory Layout:** Broadcasting is achieved by modifying stride information; no data copy is performed.
...
Unlike `broadcast`, the `broadcast_to` implemented by the Triton community requires that the rank of the tensor's shape and the target shape are identical.
