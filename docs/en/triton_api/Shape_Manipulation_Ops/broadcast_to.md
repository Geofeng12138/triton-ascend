...
- Keeps the data unchanged, only modifies the tensor's shape information
...
Unlike `broadcast`, the Triton community's implementation of `broadcast_to` requires that the rank of the tensor's shape must be equal to the rank of the target shape.
