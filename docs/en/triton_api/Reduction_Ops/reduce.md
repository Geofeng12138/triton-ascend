...
Description: `triton.language.reduce` applies `combine_fn` to reduce the input tensor along the specified `axis`, returning the reduced result tensor.
...
| `combine_fn` | `Callable` | A function to combine two groups of scalar tensors (must be decorated with `@triton.jit`) |
...
> `keep_dims=True` requires more test specifications to determine full support. Currently tested for 3D dim=2, `keep_dims=True` is supported.
