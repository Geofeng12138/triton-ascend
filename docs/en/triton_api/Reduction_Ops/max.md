...
| `axis`   | `int` | Specifies the dimension along which to perform reduction; when axis=None, reduction is performed over all axes |
| `keep_dims` | `bool` | Whether to keep the reduced dimension after reduction |
| `return_indices` | `bool` | If true, return the index corresponding to the maximum value, in addition to returning the maximum value |
