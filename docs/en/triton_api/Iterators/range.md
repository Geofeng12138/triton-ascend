...
`range` is an iterator that counts upward, similar to Python's `range()` function, but allows more parameters to be passed.
...
The functionalities related to `disallow_acc_multi_buffer`, `flatten`, `warp_specialize`, and `disable_licm` are not yet fully supported.
