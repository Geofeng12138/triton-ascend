...
Description: **Converts the indices of a row-major matrix of size `size_i × size_j`, grouped every `size_g` rows, into indices of a column-major matrix.**
...
The following example converts the indices of a row-major matrix, grouped every `size_g` rows, into indices of a column-major matrix:
