...
Description: Returns the value from tensor `x` or `y` based on the condition. When the condition is true, the value from `x` is returned; otherwise, the value from `y` is returned.
