...
Description: Returns the value from tensor `x` or `y` based on the condition. When the condition is true, returns the value from `x`; otherwise, returns the value from `y`.
...
The following example selects elements element-wise based on the condition `X < Y`: takes the element from `X` when the condition is true, otherwise takes the constant `1`.
