...
Conclusion: The parameters `start` and `end` of `arange` must be constants, therefore they have no type. The supported value range is up to `int32`, and hardware instructions only support up to `int32`.
