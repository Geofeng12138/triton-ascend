...
**Note: When `propagate_nan=tl.PropagateNAN.NONE`, the system will automatically add NaN value handling logic, which may lead to:**

1. **Increased UB space usage**: Additional NaN detection and handling requires more UB space
2. **Possible performance degradation**: Due to the added computation logic, operator execution performance may decrease
