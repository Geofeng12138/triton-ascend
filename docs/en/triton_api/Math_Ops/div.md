...
The underlying implementation is the same as the `fdiv` operator, except that `fdiv` explicitly restricts input parameters to float types, while '/' has no such restriction. It converts non-floating-point types to floating-point types before computation.
