...
Introduction: Subtraction, arithmetic operation '-', no `tl.sub` method.
...
Return value:
`tl.tensor`: Result of subtraction
...
The following example demonstrates performing subtraction on input tensors `in_ptr0` and `in_ptr1`:
