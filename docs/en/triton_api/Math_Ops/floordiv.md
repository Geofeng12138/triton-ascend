...
Introduction: Integer division, returns the division result rounded toward zero, corresponding to the arithmetic operator '//'. There is no `tl.floordiv` method.
...
The following example implements integer division on input tensors `in_ptr0` and `in_ptr1`:
