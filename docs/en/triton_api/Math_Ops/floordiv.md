...
Introduction: Floor division, returns the division result rounded towards zero, corresponding to the arithmetic operator '//'. There is no `tl.floordiv` method.
...
The following example performs floor division on input tensors `in_ptr0` and `in_ptr1`:
