...
- `kwargs` – A dictionary of 1-tuple arguments to be passed to the kernel as keyword arguments.

- `num_warps` – The number of warps used by the kernel when compiling for GPU. For example, if num_warps=8, each kernel instance will be automatically parallelized and executed cooperatively by 8 * 32 = 256 threads.
