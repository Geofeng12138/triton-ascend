...
- Note: When all configs are evaluated, the kernel will be run multiple times. This means any values updated by the kernel will be updated multiple times. To avoid this undesirable behavior, use the `reset_to_zero` parameter, which resets the provided tensor values to zero before running any config.
...
- `restore_value (list[str])` - A list of parameter names whose values will be restored after any config is evaluated.
