...
Description: Performs a left bitwise shift on a tensor by a given value.
...
2. The right operand `other` only supports scalars, not tensors (i.e., `x << 2` is valid, `x << y` (where `y` is a tensor) is currently not supported).
