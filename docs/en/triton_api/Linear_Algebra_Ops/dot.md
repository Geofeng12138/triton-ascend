...
| `acc`                | `int32 float32`     | Accumulator tensor. If not None, the result is added to this tensor. Supported `acc_dtype` values: `float16`, `float32`, `int32`. |
| `input_precision`    | -                    | Available options for NVIDIA. Selects the precision mode to enable Tensor Cores acceleration.                                     |
...
- `acc` cannot support `fp16`; the hardware defaults to `fp32` for precision.
