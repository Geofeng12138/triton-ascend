...
本文主要介绍在 Ubuntu 环境中如何通过 Docker 镜像快速完成 **Triton-Ascend** 基础配套的安装，如果需要详细的操作步骤请参考[<u>《安装介绍》</u>](#InstallationGuide)。
...
### 检查安装环境<a id = "CheckEnvironment" ></a>
...
### 检查安装环境
确定 CANN、Python 和 torch_npu 软件版本并安装，软件包安装和源码编译安装均需要先完成这一步。具体参考软件包安装的[<u>环境准备事项</u>](#CheckEnvironment)
